function [Q,Qdot] = External_Moment( x )
% clear all;
% clc;
close all;
global n_d
...
eps1=eye(n_d)*1e-5; eps2=eye(n_q)*1e-5;  eps=1e-5;
...
cc=0; init=[0,0.0011,-0.0023,0.003,-0.0046,-0.0029,0.0028];
%% gradiant projection:
step=5/round(x);
t=0:step:5;
for tt=t
...
%                         ddbtoq=ddbtoq+Diadic(What',Diadic(gradf(:,Len{ii}(jj)),gradf(:,Len{ii}(ss))));
        ...
        Ip(:,:,3)=[0,0,0;0,0,0;0,0,22.1e-3]; Ip(:,:,4)=[0,0,0;0,0,0;0,0,156.74e-3];
...
%             subplot(1,2,1);
%             cla;
            K1_q=qdot_eval*step; K1_qd=M_total\(GG_total-N_total*qdot_eval)*step;
%             q_eval=Q(fix(tt/step)+1,:)'+K1_q/2; qdot_eval=Qdot(fix(tt/step)+1,:)'+K1_qd/2;
            q_eval=Q(end,:)'+K1_q/2; qdot_eval=Qdot(end,:)'+K1_qd/2;
        elseif ll==2
            K2_q=qdot_eval*step; K2_qd=M_total\(GG_total-N_total*qdot_eval)*step;
            q_eval=Q(end,:)'+K2_q/2; qdot_eval=Qdot(end,:)'+K2_qd/2;
        elseif ll==3
            K3_q=qdot_eval*step; K3_qd=M_total\(GG_total-N_total*qdot_eval)*step;
            q_eval=Q(end,:)'+K3_q; qdot_eval=Qdot(end,:)'+K3_qd;
        else
            K4_q=qdot_eval*step; K4_qd=M_total\(GG_total-N_total*qdot_eval)*step;
        end
    end
